...
Public Class ReporteFichaOpinion
    Inherits System.Web.UI.Page
    Dim ReporteFichaOpinionV As New ReporteFichaOpinionVista
    ...
    Private Sub MostrarGrafico()
        Session("ReporteFichaOpinion") = ReporteFichaOpinionV.ReporteFichaOpinionBLL.ReporteFichaOpinion
        Dim Lista = CType(Session("ReporteFichaOpinion"), List(Of ReporteFichaOpinionENT))
        Dim x As String() = New String(Lista.Count - 1) {}
        Dim y As Decimal() = New Decimal(Lista.Count - 1) {}
        For i As Integer = 0 To Lista.Count - 1
            x(i) = Lista.Item(i).Opinion
            y(i) = Convert.ToInt32(Lista.Item(i).Cuenta)
        Next
        BarChart1.Series.Add(New AjaxControlToolkit.BarChartSeries() With {.Data = y, .Name = "Opiniones"})
        BarChart1.CategoriesAxis = String.Join(",", x)
        BarChart1.ChartTitle = "Opiniones"
...
"</style></head><body>" & hiddenHTML.Value & "</body></html>"
        ...
        'memoryStream.Close()
        doc.Save(Response, False, "ReporteFichaOpinion.pdf")
        ' close pdf document
        doc.Close()
    End Sub
